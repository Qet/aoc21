...
vector<point> get_line_points(point start, point end)
{
	...
	else //must be diagonal due to puzzle definition
	{
		vector<int> seq_x = get_coord_sequence(start.x, end.x);
		vector<int> seq_y = get_coord_sequence(start.y, end.y);
		//x and y must be the same length because its diagonal, otherwise this code will crash. 

		for (int i = 0; i < seq_x.size(); i++)
		{
			points.emplace_back(seq_x[i], seq_y[i]);
		}
